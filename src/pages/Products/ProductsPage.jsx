  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 bg-transparent font-sans w-full relative">
      ...
      <div className="flex flex-col lg:flex-row gap-4 items-start w-full flex-1 min-h-0 relative">
        ...
        {sidebarOpen && (
          <div className="fixed top-[104px] bottom-0 left-0 right-0 bg-black/60 z-[9940] lg:hidden animate-fade-in" onClick={() => setSidebarOpen(false)} />
        )}

        {/* Left Filter Sidebar */}
        <div className={`fixed top-[104px] bottom-0 left-0 z-[9950] w-80 max-w-[85vw] bg-white shadow-2xl p-4 overflow-y-auto custom-scrollbar transform transition-all duration-300 ${sidebarOpen ? 'lg:static lg:w-64 lg:p-0 lg:shadow-none lg:bg-transparent lg:overflow-y-auto lg:h-full lg:flex-none lg:translate-x-0 translate-x-0' : '-translate-x-full lg:hidden'}`}>
        ...
        <div className="flex-1 bg-white rounded-2xl shadow-sm border border-gray-100 flex flex-col overflow-hidden w-full h-full min-w-0">
          <div className="overflow-x-auto overflow-y-auto flex-1 w-full custom-scrollbar relative">
            ...
            <div className="block md:hidden flex flex-col divide-y divide-gray-100 bg-white">
              ...
              {pageItems.length === 0 && (
                <div className="p-8 text-center text-gray-400 text-xs">Không tìm thấy hàng hóa nào phù hợp</div>
              )}
            </div>
