...
export default function SettingsPage() {
  ...
  return (
    <div className="flex flex-col gap-6 animate-page-in">
      <h1 className="text-2xl font-extrabold text-gray-800 m-0 tracking-tight">Thiết lập</h1>
      <div className="flex gap-6 items-start">
        {/* Sidebar */}
        <div className="w-[260px] shrink-0 bg-white border border-gray-100 rounded-xl overflow-hidden shadow-sm flex flex-col py-2">
          {SECTIONS.map(s => {
            const Icon = s.icon;
            return (
              <button key={s.key} onClick={() => setSection(s.key)} className={`w-full flex items-center gap-3 px-5 py-3.5 text-[13px] transition-all cursor-pointer border-none text-left ${section === s.key ? 'bg-blue-50/50 text-primary font-bold border-l-[3px] border-l-primary' : 'text-gray-600 font-medium hover:bg-gray-50 hover:text-gray-800 border-l-[3px] border-l-transparent'}`}>
        ...
        {/* Content */}
        <div className="flex-1 bg-white border border-gray-100 rounded-xl p-8 shadow-sm min-h-[600px]">
          {section === 'store' && (
            <div className="animate-fade-in">
              <h2 className="text-xl font-extrabold text-gray-800 mb-6 tracking-tight">Thông tin cửa hàng</h2>
              <div className="grid grid-cols-2 gap-6 bg-gray-50/50 p-6 rounded-xl border border-gray-100">
                ...
                <div><label className="text-[13px] font-bold text-gray-700 mb-1.5 block">Mã cửa hàng</label><input className="w-full border border-gray-200 rounded-lg px-3.5 py-2.5 text-[13px] font-bold text-gray-500 bg-gray-100 cursor-not-allowed" readOnly defaultValue="TIKO001" /></div>
          ...
          {section === 'display' && (
            <div className="animate-fade-in">
              <h2 className="text-xl font-extrabold text-gray-800 mb-6 tracking-tight">Giao diện</h2>
          ...
          {section === 'notification' && (
            <div className="animate-fade-in">
              <h2 className="text-xl font-extrabold text-gray-800 mb-6 tracking-tight">Thông báo</h2>
              <div className="bg-gray-50/50 p-6 rounded-xl border border-gray-100">
          ...
          {section === 'security' && (
            <div className="animate-fade-in">
              <h2 className="text-xl font-extrabold text-gray-800 mb-6 tracking-tight">Bảo mật</h2>
              <div className="bg-gray-50/50 p-6 rounded-xl border border-gray-100">
          ...
          {!['store','display','notification','security'].includes(section) && (
            <div className="text-center py-24 text-gray-400 animate-fade-in flex flex-col items-center">
              <div className="w-20 h-20 bg-gray-50 rounded-full flex items-center justify-center mb-5">
                <SettingsIcon size={40} className="text-gray-300 animate-[spin_4s_linear_infinite]" />
              </div>
              <div className="text-lg font-bold text-gray-600 mb-2">Đang phát triển</div>
              <div className="text-sm font-medium text-gray-500">Phần cài đặt "{SECTIONS.find(s => s.key === section)?.label}" sẽ sớm ra mắt</div>
            </div>
          )}
        </div>
      </div>
