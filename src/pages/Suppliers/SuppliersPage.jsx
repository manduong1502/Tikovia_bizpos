...
import {
  Plus, Download, Search, Building2, Edit, Trash2, Star, Filter, Columns3, Settings, HelpCircle, Copy, Save, Printer, MoreHorizontal, Eye, Tag, AlertCircle, X, Upload, SlidersHorizontal, Phone, ChevronDown
} from 'lucide-react';
...
export default function SuppliersPage() {
  ...
  const handleExportDebt = async (timeRange, columns) => {
    ...
    let row6 = createRow(); row6[0] = 'CÔNG NỢ CHI TIẾT NHÀ CUNG CẤP'; exportData.push(row6);
    ...
    let row9 = createRow(); row9[0] = 'Tên NCC:'; row9[1] = s.name; row9[totalCols - 3] = 'Nợ đầu kỳ:'; row9[totalCols - 1] = Number(noDauKy || 0); exportData.push(row9);
    let row10 = createRow(); row10[0] = 'Mã NCC:'; row10[1] = supCode; row10[totalCols - 3] = 'Tổng phát sinh Nợ:'; row10[totalCols - 1] = Number(totalGhiNo || 0); exportData.push(row10);
    let row11 = createRow(); row11[0] = 'Điện thoại:'; row11[1] = s.phone || ''; row11[totalCols - 3] = 'Tổng phát sinh Có:'; row11[totalCols - 1] = Number(totalGhiCo || 0); exportData.push(row11);
                        ...
                        <div className="sticky top-0 bg-white border-b border-gray-100 px-4 py-3 flex items-center justify-between z-10 shadow-sm">
                          <div className="flex items-center gap-2">
                            ...
                            <div className="flex flex-col">
                              <span className="font-extrabold text-gray-900 text-sm">Chi tiết nhà cung cấp</span>
                              <span className="text-xs font-bold text-primary">{code} - {s.name}</span>
                            </div>
                          </div>
                          <button
                            onClick={() => setExpandedId(null)}
