...
export default function PurchaseReturnDetailModal({ open, onClose, data, partnerName, onRefresh }) {
  const navigate = useNavigate();
  if (!open || !data) return null;

  const handleCancel = async () => {
    if (!window.confirm(`Bạn có chắc chắn muốn hủy phiếu trả hàng nhập ${data.code} này? Giao dịch này sẽ bị hủy hoàn toàn, tồn kho và công nợ sẽ được hoàn lại.`)) {
      return;
    }
    try {
      const realId = typeof data.id === 'string' ? parseInt(data.id.split('-')[0], 10) : data.id;
      await purchaseReturnAPI.delete(realId);
      toast.success('Hủy phiếu trả hàng nhập thành công');
      if (onRefresh) onRefresh();
      onClose();
    } catch (err) {
      console.error(err);
      toast.error('Hủy phiếu trả hàng nhập thất bại');
    }
  };

  const items = data.items || [];
  const statusLabels = {
    'COMPLETED': { text: 'Đã trả', bg: 'bg-green-100', color: 'text-green-700' },
      ...
      <div className="bg-white rounded-xl shadow-2xl w-[900px] max-h-[90vh] overflow-y-auto flex flex-col" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div className="flex items-center gap-3">
            <h2 className="text-lg font-extrabold text-gray-800 tracking-tight">Trả hàng nhập</h2>
        ...
        <div className="p-6 flex flex-col gap-6">
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-[13px]">
            <div><span className="text-gray-500 block">Ngày trả:</span><span className="font-bold text-gray-800">{new Date(data.date || data.createdAt || data.created_at).toLocaleString('vi-VN')}</span></div>
          ...
          <div className="flex justify-end gap-16 text-[13px]">
            <div className="flex flex-col gap-2 w-64">
              <div className="flex justify-between"><span className="text-gray-500">Số lượng mặt hàng</span><span className="font-bold">{items.length}</span></div>
              <div className="flex justify-between"><span className="text-gray-500">Tổng số lượng</span><span className="font-bold">{totalQty}</span></div>
              <div className="flex justify-between"><span className="text-gray-500">Tổng tiền hàng</span><span className="font-bold">{fmt(Math.abs(data.total))}</span></div>
              <div className="flex justify-between border-t border-gray-100 pt-2"><span className="text-gray-500 font-bold">NCC đã trả</span><span className="font-bold text-green-600">{fmt(data.paid)}</span></div>
            </div>
          </div>
        </div>
